...
#include "../tof_drv/sns_tof_drv.h"
#ifdef __cplusplus
extern "C" {
#endif

#define CAMERA_CONFIG_SENSOR_NUM
#define CONFIG_VENDOR_DUMP
#ifdef CONFIG_VENDOR_DUMP
#define CONFIG_XML_PATH "/vendor/soc_platform/etc/"
#define CONFIG_JSON_PATH "/vendor/soc_platform/etc/"
#else
#define CONFIG_XML_PATH "/odm/etc/"
#define CONFIG_JSON_PATH "/odm/etc/"
#endif
...
struct camera_device_manager {
    ...
    int scan_sensor_num;  // scan sensor num actually
};
...
cmr_int sensor_get_i2c_ops_status(cmr_handle sns_module_handle);
...
cmr_int sensor_ic_write_exp_common(cmr_handle handle, cmr_uint param);

cmr_int sensor_ic_write_gain_common(cmr_handle handle, cmr_u32 param);

cmr_int sensor_ic_write_multi_ae_common(cmr_handle handle, void *param);

cmr_int sensor_af_set_pos_common(cmr_handle handle, cmr_u32 param);

cmr_int sensor_calic_sns_otp_common(struct sensor_drv_context *sensor_cxt);
...
SINGLE_LOGICAL_SENSOR_INFO_T *sensorGetSingleLogicalSnsInfo(int logical_id);
...
cmr_int sensor_af_softlanding(struct sensor_drv_context *sensor_cxt, void *slp);
cmr_int sensor_drv_get_cmei_status(void);
cmr_int sensor_drv_set_manual_cmei(cmr_u8 dual_flag, bool type);
cmr_u32 sensor_drv_get_cmei_status_bokeh_calibration(void);
cmr_u32 sensor_drv_get_cmei_status_oz1_module_snspid(void);
cmr_u32 sensor_drv_get_cmei_status_oz2_module_snspid(void);
cmr_u32 sensor_get_tof_support(cmr_u32 id);
int sensor_drv_get_scan_sensor_num(void);
cmr_int sensor_dump_csi_trace(struct sensor_drv_context *sensor_cxt);
void sensor_dump_configuration_info(int module_num, struct xml_camera_module_cfg *cfg_info);
void sensor_dump_sensor_info(struct iss_dump_sensorinfo *info, int scan_sensor_num);
void sensor_dump_otp_data(struct sensor_drv_context *sensor_cxt);
cmr_int sensor_get_virtual_channel_alignment(int camId);
#ifdef __cplusplus
}
#endif
