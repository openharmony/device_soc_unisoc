...
#include <utils/MetadataBufferType.h>
#include "refbase.h"
#include "VideoAPI.h"
#include <OMX_Component.h>
namespace OHOS {
namespace OMX {
// This structure is used to enable OMX native buffer use for either
// graphic buffers or secure buffers.
//
// TO CONTROL OMX GRAPHIC BUFFER USAGE:
//
// A pointer to this struct is passed to the OMX_SetParameter when the extension
// index for the 'OMX.index.enableNativeBuffers' extension
// is given.
//
// When native buffer use is disabled for a port (the default state),
...
// required.
//
// When native buffer use has been enabled for a given port, the video
// color format for the port is to be interpreted as an OMX pixel format
// rather than an OMX color format.  Enabling OMX native buffers may also
...
// 'OMX.index.useNativeBuffer2' extension, it should
// expect to receive UseNativeBuffer calls (via OMX_SetParameter) rather
// than UseBuffer calls for that port.
//
// TO CONTROL OMX SECURE BUFFER USAGE:
//
// A pointer to this struct is passed to the OMX_SetParameter when the extension
// index for the 'OMX.index.allocateNativeHandle' extension
// is given.
//
...
struct EnableOMXNativeBuffersParams {
    OMX_U32 nSize;
    OMX_VERSIONTYPE nVersion;
    OMX_U32 nPortIndex;
    OMX_BOOL enable;
};
typedef struct EnableOMXNativeBuffersParams AllocateNativeHandleParams;
// A pointer to this struct is passed to OMX_SetParameter() when the extension index
// "OMX.index.storeMetaDataInBuffers" or
// "OMX.index.storeANWBufferInMetadata" is given.
//
...
// directed by enable/use-native-buffer parameter settings.
...
//
// If the component supports this using the "OMX.index.storeANWBufferInMetadata"
...
//
// If the component supports this using the "OMX.index.storeMetaDataInBuffers"
...
struct VideoGrallocMetadata {
    MetadataBufferType eType;               // must be K_METADATA_BUFFER_TYPE_GRALLOC_SOURCE
#ifdef OMX_COMPILE_AS_32BIT_ON_64BIT_PLATFORMS
    OMX_PTR pHandle;
#endif
};
...
struct VideoNativeHandleMetadata {
    MetadataBufferType eType;               // must be K_METADATA_BUFFER_TYPE_NATIVE_HANDLE_SOURCE
#ifdef OMX_COMPILE_AS_32BIT_ON_64BIT_PLATFORMS
    OMX_PTR pHandle;
#endif
};
// A pointer to this struct is passed to OMX_SetParameter() when the extension
// index "OMX.index.prepareForAdaptivePlayback" is given.
//
...
// index for the 'OMX.index.getNativeBufferUsage'
// extension is given.  The usage bits returned from this query will be used to
// allocate the Gralloc buffers that get passed to the useNativeBuffer
// command.
struct GetNativeBufferUsageParams {
    OMX_U32 nSize;              // IN
    OMX_VERSIONTYPE nVersion;   // IN
    OMX_U32 nPortIndex;         // IN
    OMX_U32 nUsage;             // OUT
};
// An enum OMX_colorFormatOpaque to indicate an opaque colorformat
// This will inform the encoder that the actual
// colorformat will be relayed by the GRalloc Buffers.
// OMX_colorFormatOpaque  = 0x7F000001,
// A pointer to this struct is passed to OMX_SetParameter when the extension
// index for the 'OMX.index.prependSPSPPSToIDRFrames' extension
// is given.
...
// A pointer to this struct is passed to OMX_GetParameter when the extension
// index for the 'OMX.index.describeColorFormat'
...
// incorrect. bUsingNativeBuffers is OMX_TRUE if native buffers will
...
// buffers, the component shall set planeCount to 0, and imageType to MediaImage::kTypeUnknown.
...
// A pointer to this struct is passed to OMX_GetParameter when the extension
// index for the 'OMX.index.describeColorFormat2'
...
// 'OMX.index.configureVideoTunnelMode' extension is  given.
...
// 'OMX.index.describeColorAspects' extension is given. Component SHALL behave
...
struct DescribeColorAspectsParams {
    ...
    SprdColorAspects sAspects;         // IN/OUT
};
// HDR color description parameters.
// This is passed via OMX_SetConfig or OMX_GetConfig to video encoders and decoders when the
// 'OMX.index.describeHDRStaticInfo' extension is given and an HDR stream
...
//
// This is passed via OMX_SetConfig or OMX_GetConfig to video encoders and decoders when the
// 'OMX.index.describeHDR10PlusInfo' extension is given. In general, this config
