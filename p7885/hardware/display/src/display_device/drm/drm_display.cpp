...
std::mutex g_tpMutext;

static int32_t SetTpSuspend(uint32_t status)
{
#ifdef HIHOPE_OS_DEBUG
    HITRACE_METER(HITRACE_TAG_GRAPHIC_AGP);
#endif
    std::lock_guard<std::mutex> lock(g_tpMutext);

    static int32_t brFd = 0;
    constexpr int32_t bufferSize = 10; // buffer size
    char buffer[bufferSize];

    DISPLAY_LOGI("set tp suspend %{public}d", status);
    if (brFd <= 0) {
        brFd = open("/sys/touchscreen/ts_suspend", O_RDWR);
        if (brFd < 0) {
            DISPLAY_LOGE("open ts_suspend file failed\n");
...
int32_t DrmDisplay::SetDisplayPowerStatus(DispPowerStatus status)
{
    ...
    HITRACE_METER(HITRACE_TAG_GRAPHIC_AGP);
#endif
    DISPLAY_LOGI("the status %{public}d ", status);
    //set 0 before we enable lcd.
    if (mStatus != status and POWER_STATUS_ON == status) {
    ...
    setTpSuspendThread.join(); // about 150ms
    ...
    if (POWER_STATUS_ON == mStatus) {
        ...
        if (brightness == 0) {
            SetDisplayBacklight(defaultBrightness);
        }
    } else {
...
int32_t DrmDisplay::WaitForVBlank(uint64_t *ns)
{
    ...
    int ret = drmWaitVBlank(mDrmDevice->GetDrmFd(), &vblank);
...
#else
    usleep(1000*5);
    *ns = 1000*1000*5;  //static_cast<uint64_t>(vbl.reply.tval_sec * nPerS + vbl.reply.tval_usec * nPerUS);
#endif
    return DISPLAY_SUCCESS;
}
...
int32_t DrmDisplay::AllocMem(const AllocInfo& info, BufferHandle*& handle)
{
    ...
    void *libHandle = dlopen(libHdiImpl, RTLD_NOW | RTLD_NOLOAD);
    if (libHandle != nullptr) {
        DISPLAY_LOGD("Library '%{public}s' already loaded", libHdiImpl);
    } else {
        libHandle = dlopen(libHdiImpl, RTLD_NOW);
        if (libHandle == nullptr) {
            DISPLAY_LOGE("Failed to load library: %{public}s", dlerror());
            return DISPLAY_FAILURE;
        }
    }

    using AllocMemFunc = int32_t (*)(const AllocInfo&, BufferHandle*&);
    AllocMemFunc func = reinterpret_cast<AllocMemFunc>(dlsym(libHandle, "AllocMemVdi"));
    if (func == nullptr) {
        DISPLAY_LOGE("Failed to load %{public}s function: %{public}s", "AllocMemVdi", dlerror());
        dlclose(libHandle);
        return DISPLAY_FAILURE;
    }
...
int32_t DrmDisplay::PushFirstFrame()
{
    ...
    drmComp->Apply(true);
    drmComp->Apply(true);
    DISPLAY_LOGE("drmComp->apply done\n");
    return DISPLAY_SUCCESS;
}
...
int32_t DrmDisplay::SetDisplayVsyncEnabled(bool enabled)
{
    ...
    DISPLAY_LOGD("enable %{public}d  mStatus %{public}d ", enabled, mStatus);
    return DISPLAY_SUCCESS;
}
