...
{
    constexpr uint32_t MASTER_RETRY_COUNT = 10;
    constexpr useconds_t MASTER_RETRY_DELAY_US = 20000;
    for (uint32_t retry = 0; ; ++retry) {
        if (drmSetMaster(GetDrmFd()) == 0) {
            break;
        }
        const int masterErrno = errno;
        DISPLAY_CHK_RETURN((retry >= MASTER_RETRY_COUNT || (masterErrno != EACCES && masterErrno != EBUSY)),
