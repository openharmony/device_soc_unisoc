...
static inline const char* NnrtBasename(const char* path)
{
    const char* base = strrchr(path, '/');
    return (base != NULL) ? (base + 1) : path;
}

static inline void NnrtCheckNullPointerOrReturn(const void* pointer, const char* function, const char* file, int line)
{
    if (pointer != NULL) {
        return;
    }
    HILOG_ERROR(LOG_CORE,
        "\033[0;32;31m"
        "[%{public}s@%{public}s:%{public}d] pointer is null and return"
        "\033[m"
        "\n",
        function, NnrtBasename(file), line);
}

static inline int NnrtCheckNullPointerOrReturnValue(
    const void* pointer, const char* function, const char* file, int line)
{
    if (pointer != NULL) {
        return 0;
    }
...
#define NNRT_FILENAME (strrchr(__FILE__, '/') ? (strrchr(__FILE__, '/') + 1) : __FILE__)
    ...
    do {                                                                                                              \
        if (NNRT_DEBUG_ENABLE) {                                                                                      \
            HILOG_DEBUG(LOG_CORE, "[%{public}s@%{public}s:%{public}d] " format "\n",                                  \
                __FUNCTION__, NNRT_FILENAME, __LINE__,                                                                \
                ##__VA_ARGS__);                                                                                       \
        }                                                                                                             \
    } while (0)
#endif
...
#define NNRT_LOGE(format, ...)                                                                                        \
    do {                                                                                                              \
        HILOG_ERROR(LOG_CORE,                                                                                         \
            ...
            "\n",                                                                                                     \
            __FUNCTION__, NNRT_FILENAME, __LINE__, ##__VA_ARGS__);                                                    \
    } while (0)
#endif
