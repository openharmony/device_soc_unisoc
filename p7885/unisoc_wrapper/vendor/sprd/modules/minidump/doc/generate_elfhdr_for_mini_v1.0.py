...
def Generate_elf32_files():
    file_name = '00_minidump_elfhdr'
    file_size = '8192'
    file_path = os.path.join(file_dir, file_name)
    create_new_file(file_path, file_size)
    fd = open(os.path.join(file_dir, file_name), 'rb+')
    fd.seek(0)
    elfhdr_offset = 0
    ...
    #--------------PT_NOTE--------------------
    elf32_nhdr = ELE32_P_HEADER()
    elf32_nhdr.p_type = PT_NOTE
    elf32_nhdr.p_offset = 0
    elf32_nhdr.p_vaddr = 0
    elf32_nhdr.p_paddr = 0
    elf32_nhdr.p_filesz = 0
    elf32_nhdr.p_memsz = 0
    elf32_nhdr.p_flags = 0
    elf32_nhdr.p_align = 0
    ...
    minidump_mem_name_l = fnmatch.filter(os.listdir(file_dir), '*_mem_info')
    minidump_mem_file = open(os.path.join(file_dir, minidump_mem_name_l[0]), 'rb')
...
def handle_elf():
    ...
    file_list = fnmatch.filter(os.listdir(file_dir), '*vmcore*')
    print(file_list[0])
    vmcoreinfo_file = file_list[0]    
    infofd = open(os.path.join(file_dir, vmcoreinfo_file), 'rb+')
    for line in infofd.readlines():
        if kimage_string in line:
            Generate_elf64_files()
            return
    Generate_elf32_files()
